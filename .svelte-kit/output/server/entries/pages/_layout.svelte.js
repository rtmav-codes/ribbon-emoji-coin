...
function _layout($$payload, $$props) {
  ...
  $$payload.out += `<div class="ribbon-container svelte-5vomed"><!--[-->`;
  for (let i = 0, $$length = each_array.length; i < $$length; i++) {
    each_array[i];
    $$payload.out += `<div class="ribbon svelte-5vomed"${attr("style", `--delay: ${stringify(i * 0.5)}s; --left: ${stringify(Math.random() * 100)}%`)}></div>`;
  }
  $$payload.out += `<!--]--></div> `;
  Nav($$payload);
  $$payload.out += `<!----> <!---->`;
